...
const count = (target, input) => {
  // Solution code here...
  let counter = 0;
  let totalCounter = input.reduce( (total, numberSet) => {
...
const totalSum = (input) => {
  // Solution code here...
  let totalSum = input.reduce( (total, numberSet) => {
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let results = input.reduce ( (newArr, numberSet) => {
    let division = numberSet.filter( value => {
      if ( value % 5 === 0 && typeof value === 'number') {
        return value;
      };
      });
    newArr.push(division.map( val => 2 ** val));
    return newArr;
  }, [])
  return results;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let regex = /[,]/g;
  let charArr = data.reduce( (nameArr, char) => {
    if( char.gender === 'male' || char.gender === 'female') {
      nameArr.push(char.name);
    };
    return nameArr;
  }, []);
  return charArr.toString().replace(regex, ' and ');
};
